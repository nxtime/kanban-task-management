...
const Board = ({ columnId }: { columnId: string }) => {
  const { board: boards, selectedBoard, setCurrentSelectedTask } = useBoardStore((state) => state);
    ...
    <>
      {
        !board ? (
          <div className="flex flex-col gap-6 items-center justify-center h-full">
            <h2 className="text-heading-md font-bold text-gray">This board is empty. Create a new column to get started.</h2>
            <button className="btn btn-primary capitalize">+ add new column</button>
          </div>
        ) : (
            ...
            {board.columns.map((column, colIndex: number) => (
              <li key={column.name} className="w-[280px]">
                <div className="flex gap-2 items-center">
                  <span className={`h-4 w-4 rounded-full bg-[${colors[colIndex]}]`} style={{ backgroundColor: colors[colIndex] }} />
                  <h2 className="text-heading-sm uppercase tracking-[0.25em] text-gray font-bold">{column.name} ({column.tasks.length})</h2>
                </div>
                <ul className="mt-4 flex flex-col gap-5">
                  {column.tasks.map((task, taskIndex) => (
                    <li key={task.title} >
                      ...
                      >
                        <h2 className="font-bold text-heading-md text-start">{task.title}</h2>
                        <span className="text-gray font-bold text-body-md">
                          {task.subtasks.filter(({ isCompleted }) => isCompleted).length} of {task.subtasks.length} subtasks
                        </span>
                      </button>
                    </li>
                  ))}
